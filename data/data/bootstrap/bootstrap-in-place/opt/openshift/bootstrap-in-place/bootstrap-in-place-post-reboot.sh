...
function restart_kubelet {
  ...
  while grep  bootstrap-kube-apiserver /etc/kubernetes/manifests/kube-apiserver-pod.yaml ; do
...
function approve_csr {
  ...
  until [ "$(oc get nodes | grep master | grep -v NotReady | grep -c Ready)" -eq 1 ]; do
