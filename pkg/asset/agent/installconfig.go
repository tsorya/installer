...
func (a *OptionalInstallConfig) validatePlatformsByName(installConfig *types.InstallConfig) field.ErrorList {
	...
	if installConfig.Platform.Name() == external.Name {
		if installConfig.Platform.External.PlatformName == "oci" &&
			...
			allErrs = append(allErrs, field.Invalid(fieldPath, installConfig.Platform.External.CloudControllerManager, fmt.Sprintf("When using external %s platform, %s must be set to %s", "oci", fieldPath, external.CloudControllerManagerTypeExternal)))
		}
	}
